...
采集 Chrome 是由运营台启动的可见普通 Chrome，使用独立且持久的资料目录。VPN、登录、验证码、安全验证以及验证页刷新全部由运营人员人工完成；程序只识别并暂停，不点击、不刷新、不绕过验证。商品详情明确显示售罄或链接失效时，系统会写入商品可用性状态并从后续评论队列排除；下次重新采集 Top Sales 时，如商品仍在列表中会恢复为可用。不要连接或复制日常主浏览器资料目录。
