...
```powershell
npm run reviews:quick
npm run reviews:deep
...
采集 Chrome 是由运营台启动的可见普通 Chrome，使用独立且持久的资料目录。VPN、登录、验证码、安全验证以及验证页刷新全部由运营人员人工完成；程序只识别并暂停，不点击、不刷新、不绕过验证。采集 Chrome 若显示“售罄/不可购买”，系统仅记录为“当前采集会话不可售”，从本轮评论队列跳过，不把它认定为商品全局售罄；可用日常 Chrome 打开同一链接复核。链接失效才会作为长期排除状态。下次重新采集 Top Sales 时，如商品仍在列表中会恢复为可用。不要连接或复制日常主浏览器资料目录。
...
3. 1000个商品池：采集商品基础指标并分批轻扫近30天评论，用于7/30/90天活跃度；不抓全部历史评论。
